...
def download_test_data():
    ...
    for control_file in control_files:
        if not control_file["downloaded"]:
            ...
            if os.path.isfile(os.path.join(WORKING_DIR, control_file["name"])):
                os.remove(os.path.join(WORKING_DIR, control_file["name"]))
    ...
    if not test_data_downloaded:
        ...
        shutil.copytree(
            ...
            os.path.join(WORKING_DIR, "eqcorrscan", "tests", "test_data"))
        if os.path.isdir(TEST_DATA_PATH):
            shutil.rmtree(TEST_DATA_PATH)
        shutil.copytree(
            "EQcorrscan-{0}/eqcorrscan/tests/test_data".format(VERSION),
...
def run_tests(arg_list):
    ...
    with cd(WORKING_DIR):
        ...
        # Run the tests!
        # For version < 0.3.3 utils.correlate_test has to be run separately
        if int(VERSION.split(".")[1]) < 3 or (
                int(VERSION.split(".")[1]) == 3 and
                int(VERSION.split(".")[2]) < 3):
            ret = pytest.main(
                args=arg_list + 
                ["--ignore", PKG_PATH + "/tests/correlate_test.py"])
            ret += pytest.main(
                args=[PKG_PATH + "/tests/correlate_test.py", "--cov-append"])
        else:
